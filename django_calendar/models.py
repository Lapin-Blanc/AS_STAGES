...
class Traineeship(models.Model):
    ...
    def clean(self):
        ...
        if not self.is_closed and Traineeship.objects.exclude(id=int(self.id or -1)).filter(student=self.student, is_closed=False):
            raise ValidationError(
                "Il existe déjà un stage d'ouvert pour cet étudiant : %(ts)s",
                code = 'invalid',
                params = {'ts':Traineeship.objects.get(student=self.student, is_closed=False)}
            )
